...
主たる介入（L1→L3）では、キーパーソン本人の変わり方はほぼ同じだったのに、
...
| 追試で使ったモデル | `claude-opus-5`（`--model` で指定、`--effort low`）<br>※この課題では思考が一度も発生しておらず（thinking_tokens=0）、effort設定は結果に影響していません |
| 採点する審査役のモデル | **常に** `claude-sonnet-4-5-20250929`（追試でも据え置き） |
| 1発言あたりの上限 | `max_tokens=300` |
| 構成 | 4エージェント・3フェーズ・各フェーズ3ラウンド |
| 実行回数 | **計59回**（条件A/B/C比較 15回 ／ 自律性レベル 20回 ／ 追試 24回）<br>自律性レベルのL1は条件Cと設定が同じため、条件比較の5回を流用しています |
